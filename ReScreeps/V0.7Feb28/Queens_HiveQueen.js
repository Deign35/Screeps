...
const HiveQueenBase_1 = require("Queens_HiveQueenBase");
class HiveQueen extends HiveQueenBase_1.HiveQueenBase {
    CheckForSpawnRequirements() {
        for (let i = 0, length = this.CreepConsulList.length; i < length; i++) {
            if (this.CreepConsulList[i].CreepRequested) {
                if (!this.Spawner.ActiveSpawnNames[this.CreepConsulList[i].CreepRequested]) {
    ...
    ReleaseControl(creep) {
        this.Upgrader.AssignCreep(creep);
    }
}
exports.HiveQueen = HiveQueen;
